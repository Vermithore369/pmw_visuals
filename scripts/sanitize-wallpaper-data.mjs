...
function sanitizeFile(fileName, globalName, options = {}) {
  ...
  const sanitized = items.map((item, index) => {
    ...
    const accessType = String(
      next.accessType || next.access || (next.isPremium || next.premium ? "premium" : "free")
    ).toLowerCase();

    if (options.premiumOnly && accessType !== "premium") {
      return next;
    }
